...
export default {
  ...
  methods: {
    ...
    async loadTasks() {
      ...
      this.error = null;
      try {
        // 🔥 herkes tüm taskları görebilsin diye all=1
        const res = await axios.get("/api/tasks/?all=1", {
    ...
    async saveTitle(task) {
      ...
      task.title = newTitle; // optimistic UI
      this.cancelTitleEdit();
    ...
    async loadComments() {
      ...
      try {
        const res = await axios.get(
          `/api/tasks/${this.selectedTask.id}/comments/`,
          { withCredentials: true }
        );
